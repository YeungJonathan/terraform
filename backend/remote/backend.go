...
func (b *Remote) State(workspace string) (state.State, error) {
	...
	if !exists {
		...
		// We only set the Terraform Version for the new workspace
		// if this is a final release. When the version is omitted
		// the latest available version will be used.
		if version.Prerelease == "" {
			options.TerraformVersion = tfe.String(version.Version)
		}
